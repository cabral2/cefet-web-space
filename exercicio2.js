...
const imagensWithServer = imagens.map((img) => ({
  ...img,
  arquivo: servidorDasImagens + img.arquivo,
}));

let nextButton = document.querySelector("#proximo");
let previousButton = document.querySelector("#anterior");

const handleClick = (id) => {
  if (id === "proximo") imagensIndex += 1;

  if (id === "anterior") {
    imagensIndex += imagensWithServer.length - 1;
  }

  let imgField = document.querySelector("#slide");

  imgField.src =
    imagensWithServer[imagensIndex % imagensWithServer.length].arquivo;
  imgField.alt =
    imagensWithServer[imagensIndex % imagensWithServer.length].descricao;
};
